class FileHandler {
  ...
  getExportDirectory(): string {
    ...
    return "file_exports"; // Relative to Downloads folder
  }
  ...
  async openFile(filePath: string): Promise<void> {
    try {
      if (!window.backendAPI || !window.backendAPI.openFile) {
        console.warn("openFile API not available, file path:", filePath);
        return;
      }
      const response = await window.backendAPI.openFile(filePath);
      if (!response.status) {
