...
const TopBar: React.FC<TopBarProps> = ({ toggleSidebar }) => {
  ...
  const getRouteIcon = (category: string) => {
    switch (category) {
      case "Main": return HandCoins;
      case "Debtors": return User;
      case "Loans": return HandCoins;
      case "Collections": return DollarSign;
        ...
        <div className="hidden md:flex items-center gap-3 ml-2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--card-secondary-bg)] border border-[var(--border-color)]">
            ...
            <div className="flex flex-col">
              <div className="text-sm font-medium text-[var(--sidebar-text)]">{today.toLocaleDateString("en-US", { weekday: "long" })}</div>
        ...
        <div className="relative">
          <form onSubmit={handleSearch}>
            <div className="relative">
              ...
              />
              {searchQuery && (
                <button type="button" onClick={() => setSearchQuery("")} className="absolute inset-y-0 right-0 flex items-center pr-3">
          ...
          {showSearchResults && filteredRoutes.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 rounded-lg shadow-lg bg-[var(--sidebar-bg)] border border-[var(--border-color)] max-h-80 overflow-auto z-50">
              <div className="p-2 border-b border-[var(--border-color)]">
                <div className="text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider px-2 py-1">Quick Navigation</div>
              </div>
              {filteredRoutes.map((route, index) => {
                const RouteIcon = getRouteIcon(route.category);
                const categoryColor = route.category === "Debtors" ? "var(--accent-blue)" : route.category === "Loans" ? "var(--primary-color)" : route.category === "Collections" ? "var(--accent-green)" : route.category === "Reports" ? "var(--accent-purple)" : "var(--primary-color)";
                return (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center group-hover:scale-105 transition-transform" style={{ backgroundColor: categoryColor + "20", color: categoryColor }}>
                        <RouteIcon className="w-4 h-4" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-[var(--sidebar-text)] truncate text-sm">{route.name}</div>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs px-1.5 py-0.5 rounded bg-[var(--card-secondary-bg)] text-[var(--text-tertiary)]">{route.category}</span>
                          <span className="text-xs text-[var(--text-tertiary)] truncate">{route.path}</span>
                        </div>
                      </div>
          ...
          {showSearchResults && searchQuery.trim() && filteredRoutes.length === 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 rounded-lg shadow-lg bg-[var(--sidebar-bg)] border border-[var(--border-color)] p-6 z-50">
              <div className="text-center">
                ...
                <div className="text-[var(--sidebar-text)] font-medium mb-1">No results found</div>
                <div className="text-xs text-[var(--text-tertiary)]">Try searching for debtors, loans, payments, or reports</div>
              </div>
            </div>
          )}
        </div>
