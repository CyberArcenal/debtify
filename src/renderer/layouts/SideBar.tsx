...
const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const collectionRate = totalCollectedAndOutstanding > 0 ? (totalPaymentsCollected / totalCollectedAndOutstanding) * 100 : 0;
        ...
        debtsRes.data.forEach((debt) => {
          if (debt.status !== "paid" && debt.borrower?.id) activeDebtorIds.add(debt.borrower.id);
        });
        if (mounted) setStats({ totalOutstanding: totalRemainingBalance, overdueAmount: totalOverdue, collectionRate, activeDebtors: activeDebtorIds.size });
  ...
  const filteredMenu = menuItems.map(item => {
    if (item.children) return { ...item, children: item.children.filter(child => child.path !== "/users") };
    return item;
  }).filter(item => item.path !== "/users" && (item.children ? item.children.length > 0 : true));

  const toggleDropdown = (name: string) => setOpenDropdowns(prev => ({ ...prev, [name]: !prev[name] }));
  const isActivePath = (path: string) => location.pathname === path;
  const isDropdownActive = (items: MenuItem[] = []) => items.some(item => isActivePath(item.path));

  useEffect(() => {
    filteredMenu.forEach(item => {
      if (item.children && isDropdownActive(item.children)) setOpenDropdowns(prev => ({ ...prev, [item.name]: true }));
    });
  }, [location.pathname]);
  ...
  const renderMenuItems = (items: MenuItem[]) => {
    return items.map(item => {
      ...
      const is_active = hasChildren ? isDropdownActive(item.children) : isActivePath(item.path);
        ...
        <li key={item.path || item.name} className="mb-1">
          {hasChildren ? (
            <>
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`w-5 h-5 ${is_active ? "text-white" : "text-[var(--sidebar-text)] group-hover:text-[var(--primary-color)]"}`} />
                  <span className="font-medium">{item.name}</span>
                </div>
                <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isOpen ? "rotate-180" : ""} ${is_active ? "text-white" : "text-[var(--sidebar-text)] group-hover:text-[var(--primary-color)]"}`} />
              </div>
              {isOpen && (
                <ul className="ml-4 mt-1 space-y-1 border-l-2 pl-3" style={{ borderColor: "var(--primary-color)" }}>
            ...
            >
              ...
              <ChevronRight className={`w-4 h-4 transition-opacity duration-200 ${is_active ? "opacity-100 text-white" : "opacity-0 group-hover:opacity-50 text-[var(--sidebar-text)]"}`} />
            </Link>
          )}
        </li>
      );
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 border-b border-[var(--sidebar-border)] bg-[var(--card-secondary-bg)] p-5">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-[var(--primary-color)] to-[var(--primary-hover)] flex items-center justify-center shadow-md">
            <HandCoins className="w-6 h-6 text-white" />
          </div>
          <div className="min-w-0">
            <h2 className="truncate text-lg font-bold text-[var(--sidebar-text)]">{companyName}</h2>
            <p className="text-xs text-[var(--text-tertiary)]">Debt Management System</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="bg-[var(--status-overdue-bg)] text-[var(--status-overdue)] text-xs py-2 px-2 rounded-lg text-center border border-[var(--border-light)]">
            <div className="font-bold text-sm">
              {statsLoading ? <div className="animate-pulse h-4 w-16 bg-gray-300 rounded mx-auto"></div> : formatCurrency(stats.totalOutstanding)}
            </div>
          ...
          <div className="bg-[var(--status-partial-bg)] text-[var(--status-partial)] text-xs py-2 px-2 rounded-lg text-center border border-[var(--border-light)]">
            <div className="font-bold text-sm">
              {statsLoading ? <div className="animate-pulse h-4 w-16 bg-gray-300 rounded mx-auto"></div> : formatCurrency(stats.overdueAmount)}
            </div>
          ...
          <div className="bg-[var(--status-paid-bg)] text-[var(--status-paid)] text-xs py-2 px-2 rounded-lg text-center border border-[var(--border-light)]">
            <div className="font-bold text-sm">
              {statsLoading ? <div className="animate-pulse h-4 w-12 bg-gray-300 rounded mx-auto"></div> : `${stats.collectionRate.toFixed(1)}%`}
            </div>
          ...
          <div className="bg-[var(--accent-blue-light)] text-[var(--accent-blue)] text-xs py-2 px-2 rounded-lg text-center border border-[var(--border-light)]">
            <div className="font-bold text-sm">
              {statsLoading ? <div className="animate-pulse h-4 w-8 bg-gray-300 rounded mx-auto"></div> : stats.activeDebtors}
            </div>
      ...
      <div className="p-4 border-t border-[var(--border-color)] text-center flex-shrink-0 bg-[var(--card-secondary-bg)]">
        <p className="text-xs text-[var(--text-tertiary)] mb-2">{version} • © {new Date().getFullYear()} {toTitleCase(name)}</p>
        <div className="flex justify-center gap-4">
          <button onClick={() => navigate("/help")} className="text-[var(--text-tertiary)] hover:text-[var(--primary-color)] hover:bg-[var(--primary-color)]/10 p-1.5 rounded-full transition-colors" title="Help">
            <HelpCircle className="w-4 h-4" />
          </button>
          <Link to="system/settings" className="text-[var(--text-tertiary)] hover:text-[var(--primary-color)] hover:bg-[var(--primary-color)]/10 p-1.5 rounded-full transition-colors" title="Settings">
