...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({ isOpen, transaction, onClose, onConfirm, loading }) => (
  <Modal isOpen={isOpen} onClose={onClose} title="Delete Transaction" size="sm">
    <div className="space-y-4">
      <p style={{ color: "var(--text-primary)" }}>Are you sure you want to delete this transaction?</p>
      <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Reference: {transaction?.reference || "N/A"}<br />Amount: {transaction?.amount}</p>
