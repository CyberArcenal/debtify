...
import { useDebtDashboardData, type DebtDashboardData } from '../hooks/useDebtDashboardData';
...
const KPICard: React.FC<{
  ...
  >
    ...
    {footer && <div className="mt-3 pt-3 border-t border-[var(--border-color)]">{footer}</div>}
  </div>
);
...
const QuickActions: React.FC = () => {
  ...
  const actions = [
    { label: 'Debtors', path: '/debtors/list', icon: Users, color: 'var(--accent-blue)' },
    { label: 'New Loan', path: '/loans/applications', icon: Plus, color: 'var(--accent-green)' },
    { label: 'Record Payment', path: '/payments/transactions', icon: Wallet, color: '#f97316' },
    { label: 'Reports', path: '/reports/collection', icon: Target, color: 'var(--accent-purple)' },
  ];
  return (
...
const AgingBuckets: React.FC<{
  ...
  >
    <div className="flex items-center justify-between mb-4">
      <h3 className="font-semibold flex items-center gap-1.5" style={{ color: 'var(--sidebar-text)' }}>
...
const TopDebtorsList: React.FC<{
  ...
  >
    <div className="flex items-center justify-between mb-4">
      ...
      <button onClick={onViewAll} className="text-xs hover:underline" style={{ color: 'var(--primary-color)' }}>
        ...
        >
          <div>
            ...
            <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              {debtor.daysOverdue > 0 ? `${debtor.daysOverdue} days overdue` : 'Current'}
            </div>
          </div>
          <div className="text-right">
            <div
              className="text-sm font-bold"
              style={{ color: debtor.daysOverdue > 0 ? 'var(--accent-red)' : 'var(--sidebar-text)' }}
            >
      ...
      <div className="compact-card rounded-lg" style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)' }}>
        <div className="flex justify-center items-center h-48">
          <div className="text-center">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 mx-auto mb-3" style={{ borderColor: 'var(--primary-color)' }} />
    ...
    return (
      <div className="compact-card rounded-lg" style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)' }}>
        <div className="text-center p-6" style={{ color: 'var(--danger-color)' }}>
          ...
          <button onClick={refetch} className="btn btn-primary btn-sm rounded-md flex items-center mx-auto">
    ...
    <div className="space-y-4 transition-all duration-300 ease-in-out m-1">
        ...
        <div className="flex justify-between items-center p-4">
          <div>
            <h2 className="text-lg font-semibold flex items-center gap-1.5" style={{ color: 'var(--sidebar-text)' }}>
              <div className="w-1.5 h-5 rounded-full" style={{ backgroundColor: 'var(--primary-color)' }} />
            ...
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              ...
              <span
                ...
                style={{ background: 'var(--card-secondary-bg)', color: 'var(--primary-color)' }}
              >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{ background: 'var(--card-hover-bg)', color: 'var(--sidebar-text)' }}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        <KPICard
          ...
          icon={<HandCoins className="w-5 h-5" style={{ color: 'var(--primary-color)' }} />}
          ...
          footer={
            <div className="flex justify-between text-xs">
              <span className="cursor-pointer hover:underline" style={{ color: 'var(--primary-color)' }}>
        ...
        <KPICard
          ...
          icon={<AlertTriangle className="w-5 h-5" style={{ color: 'var(--accent-red)' }} />}
        ...
        <KPICard
          ...
          icon={<Percent className="w-5 h-5" style={{ color: 'var(--accent-green)' }} />}
          bgColor="rgba(16,185,129,0.2)"
          color="var(--accent-green)"
          badge={{
            text: collectionRate >= 70 ? `${collectionRate.toFixed(0)}% ↑` : `${collectionRate.toFixed(0)}% ↓`,
            color: collectionRate >= 70 ? 'var(--accent-green)' : 'var(--status-partial)',
          }}
        ...
        <KPICard
          ...
          value={activeDebtors}
          icon={<Users className="w-5 h-5" style={{ color: 'var(--accent-blue)' }} />}
          bgColor="rgba(59,130,246,0.2)"
          color="var(--accent-blue)"
          onClick={() => navigate('/debtors/list')}
          badge={{ text: 'Active', color: 'var(--accent-blue)' }}
          footer={<div className="text-xs">Total borrowers: {stats.totalBorrowers}</div>}
        />
      </div>
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            >
              ...
              <div className="text-lg font-bold" style={{ color: 'var(--sidebar-text)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <AgingBuckets buckets={agingBuckets} onClick={() => navigate('/reports/aging')} />
        <TopDebtorsList
          debtors={topDebtors}
          onViewAll={() => navigate('/debtors/list')}
          onSelect={(id) => navigate(`/debtors/view/${id}`)}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div
          ...
          style={{ background: 'var(--card-secondary-bg)', border: '1px solid var(--border-color)' }}
        >
          <h3 className="font-semibold flex items-center gap-1.5 mb-4" style={{ color: 'var(--sidebar-text)' }}>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center p-2 rounded-md hover:bg-[var(--card-hover-bg)]">
              <span className="text-xs flex items-center gap-1.5" style={{ color: 'var(--sidebar-text)' }}>
                <Wallet className="w-3 h-3" /> Avg. Collection per Debtor
              </span>
              <span className="text-xs font-medium" style={{ color: 'var(--accent-blue)' }}>
                {formatCurrency(stats.totalPaymentsCollected / (activeDebtors || 1))}
              </span>
            </div>
            <div className="flex justify-between items-center p-2 rounded-md hover:bg-[var(--card-hover-bg)]">
              <span className="text-xs flex items-center gap-1.5" style={{ color: 'var(--sidebar-text)' }}>
              ...
              <span className="text-xs font-medium" style={{ color: 'var(--accent-red)' }}>
                {((overdueAmount / (totalOutstanding + overdueAmount)) * 100).toFixed(1)}%
              </span>
            </div>
            <div className="flex justify-between items-center p-2 rounded-md hover:bg-[var(--card-hover-bg)]">
              <span className="text-xs flex items-center gap-1.5" style={{ color: 'var(--sidebar-text)' }}>
              ...
              <span
                ...
                style={{ color: collectionRate > 70 ? 'var(--accent-green)' : '#f59e0b' }}
              >
